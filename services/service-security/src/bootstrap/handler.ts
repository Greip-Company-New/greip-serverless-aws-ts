...
import { BASE_PERMISSIONS, BASE_PERMISSIONS_CATALOG } from '../common/constants';
...
export async function handler(_event: any = {}): Promise<any> {
  ...
  // 0. Asegurar que existan los permisos base del tenant (idempotente,
  //    ON CONFLICT DO NOTHING). Permite que permisos agregados despues del
  //    deploy inicial (ej. notify.config.*) queden disponibles en el rol ADMIN.
  for (const code of BASE_PERMISSIONS) {
    const meta = BASE_PERMISSIONS_CATALOG[code] || { name: code, description: code };
    await rbac.createPermission(
      { tenant_id: tenant.id, code, name: meta.name, description: meta.description },
      'SYSTEM'
    );
  }
  console.log(`[bootstrap] permisos base verificados para el tenant ${tenant.code} (${BASE_PERMISSIONS.length})`);

  // 1. Rol ADMIN con todos los permisos activos del tenant. Si el rol ya
  //    existia (bootstrap previo), se re-enlazan los permisos activos para que
  //    permisos agregados despues, como notify.config.*, queden disponibles;
  //    linkPermissionsToRole es idempotente (ON CONFLICT DO NOTHING).
  ...
  if (!adminRole) {
    ...
    console.log(`[bootstrap] rol ADMIN creado (id=${adminRole.id})`);
  } else {
  ...
  console.log(`[bootstrap] permisos activos enlazados al rol ADMIN (id=${adminRole.id})`);
