...
export class UsuarioRepository {
  ...
  /**
   * Actualiza atributos anidados (ej. mfa.totp.active) en el registro del
   * usuario usando una UpdateExpression con rutas de punto.
   */
  async actualizarMfa(userId: string, campos: Record<string, any>): Promise<void> {
    const entradas = Object.entries(campos);
    const names: Record<string, string> = {};
    const values: Record<string, any> = {};
    const sets: string[] = [];
    entradas.forEach(([ruta, valor], i) => {
      const segs = ruta.split('.');
      const refs = segs.map((s) => {
        const key = `#m_${s}`;
        if (!names[key]) names[key] = s;
        return key;
      });
      values[`:v${i}`] = valor;
      sets.push(`${refs.join('.')} = :v${i}`);
    });
