...
CREATE TABLE IF NOT EXISTS greip.tenant (
    ...
    formato_fecha_hora  VARCHAR(20)  NOT NULL DEFAULT 'DD/MM/YYYY HH:mm',
    formato_decimales   VARCHAR(10)  NOT NULL DEFAULT '#,##0.00',
...
FROM greip.tenant t,
     (VALUES
        ...
        ('notify.config.read', 'Leer config de notificaciones', 'Permite consultar la configuracion de proveedores de notificacion del tenant'),
        ('notify.config.manage', 'Gestionar config de notificaciones', 'Permite registrar y modificar los parametros de notificacion del tenant')
