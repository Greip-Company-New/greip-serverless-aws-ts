// Validador generico de tokens JWT RS256 emitidos por service-security.
// Reutilizable desde cualquier Lambda/servicio: verifica firma, expiracion,
// issuer y descifra el payload cifrado con AES.
import jwt from 'jsonwebtoken';
import { SecretsManagerService } from './secretsmanager.js';
import { decryptAes } from './aes.js';
...
function getSecretsManager(): SecretsManagerService {
  if (!secretsManager) {
...
async function getPublicKey(): Promise<string> {
  const secret = await getSecretsManager().getSecretValue(process.env.SM_JWT_PUBLIC_KEY || 'Greip/JWT-Public');
...
async function decryptData(data: string): Promise<any> {
  const config = await getSecretsManager().getSecretValue(process.env.SM_ENCRIPTACION_TOKEN || 'Greip/Encriptacion/Token');
